...
export const runDownload = async (sourceUri: string, credent: ServiceAccount, type: string, archive: archiver.Archiver) => {
    ...
    try {
        ...
        if (type === "rtdb") await client.database?.app.delete();
        else await client.firestore?.terminate();
    }
};

const downloadRealTimeDatabase = async (client: IInitializeApp, archive: archiver.Archiver) => {
    if (!client) return;
...
const downloadFirestore = async (client: IInitializeApp, archive: archiver.Archiver) => {
