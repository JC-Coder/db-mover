...
const HOME_SEO = getStaticRouteSeo("/") as IPageSeo;
const HERO_BRAND_LOOP = [...DATABASE_BRANDS, ...DATABASE_BRANDS];

export function LandingPage({ onStart }: ILandingPageProps) {
  ...
  return (
    <div className="min-h-full bg-[var(--landing-bg)] text-[var(--landing-text)] transition-colors duration-500 ease-out">
      <Seo
        {...HOME_SEO}
        structuredData={[softwareApplicationJsonLd(), organizationJsonLd()]}
      />
      ...
      <header className="fixed top-6 left-1/2 z-50 w-[calc(100%-1.5rem)] max-w-5xl -translate-x-1/2 sm:w-[calc(100%-3rem)]">
        ...
        {mobileMenuOpen && (
          <div className="mt-3 rounded-2xl border border-[var(--landing-border)] bg-[var(--landing-panel)] p-2 shadow-xl backdrop-blur-md md:hidden">
            ...
            <Link to="/docs" onClick={() => setMobileMenuOpen(false)} className="flex min-h-11 items-center rounded-xl px-4 text-sm font-medium text-[var(--landing-muted)] hover:bg-[var(--landing-card-soft)] hover:text-[var(--landing-text)]">Docs</Link>
